...
We structure the data such that every unordered pair is evaluated in both orientations, which balances item identity across first and second positions and makes $\gamma_k$ estimable from direct orientation reversals. Further, we enforce that (1) the sum of $\lambda_i$ across all items is zero, and (2) the geometric mean of $\alpha_k$ across all models is one, which identifies the global $\alpha-\lambda$ scale without choosing an arbitrary reference model. We discuss the full model details in the Appendix, including the hierarchical priors for $\lambda_i$, $\alpha_k$, and $\gamma_k$ in Appendix \ref{app:bradley_terry} as well as model diagnostic checks in Appendix \ref{app:diagnostic} and sensitivity analysis in Appendix \ref{app:sensitivity}. 
...
The contrast between consensus and validity is particularly important. The LLMs usually produced transitive judgments and agreed strongly with one another, but their shared ranking aligned only moderately with IRT difficulty. Agreement among artificial raters can therefore indicate a stable model-derived construct without establishing that this construct is the one measured in human response data. Models may converge on salient textual cues, curricular content, or solution complexity while missing factors that shape empirical difficulty, such as examinee familiarity, distractor functioning, instructional exposure, or population-specific misconceptions. Increasing the number of similar raters may sharpen such a shared ordering without correcting a common construct mismatch.

The rater parameters help explain why hierarchical aggregation is preferable to selecting one model or averaging rankings without adjustment. LLMs differed markedly in how sharply they responded to latent item-strength differences, and several showed presentation-order effects large enough for their 94\% intervals to exclude zero. Balanced pair orientation made these biases identifiable; without reversal, position preference could be absorbed into item strength and distort the consensus ranking. More generally, the results argue for treating LLMs as heterogeneous judges whose decisions require measurement, rather than as exchangeable replications of a single evaluator.

Performance differences across datasets also caution against broad domain-level claims. The mathematics set showed the strongest external alignment, but the literacy datasets ranged from weak to moderate despite their related content. Item count contributes to uncertainty, especially for \texttt{gilbert\_meta\_104}, but sample size alone cannot explain all heterogeneity. Differences in item format, construct representation, response processes, and the correspondence between textual demand and empirical difficulty are plausible sources that should be tested directly as access to item text grows.

For assessment development, these findings support a limited but practical role for LLM-derived rankings. They can provide immediate provisional ordering for item review, rough form assembly, or prioritization before respondent data are available. They should not be used as final calibration parameters or as a substitute for field testing. Stronger applications would combine balanced presentation, multiple model families, inspection of rater-specific $\alpha_k$ and $\gamma_k$, and anchor items with known IRT parameters. Validation should ultimately be conducted on held-out comparisons and new item sets, with respondent calibration retained as the criterion.
...
This study has several limitations. We restricted the analysis to IRW datasets with available item text; although the warehouse's available item text data is growing, the resulting set remains limited. Additional datasets and domains are needed to assess the generalizability of the findings. The evaluated LLMs are also proprietary and subject to change, which may limit reproducibility. Future studies should include open-source models, document model versions and configurations, and, as the number of models and datasets grows, examine relationships between model characteristics and performance.

The hierarchical Bradley--Terry model further assumes a particular parametric form for rater discrimination and position bias. Comparing this specification with alternative aggregation models would clarify the robustness of the findings. Finally, we held the prompt design fixed and therefore cannot determine how sensitive LLM judgments are to prompt wording or structure; systematically varying the prompts is an important direction for future work.
